...
class View():
    ...
    # create a new rendering function for jinja template 
    def render_jinja_template(self, filename, header="header", tailer="tailer", **kwargs):
        env = jinja2.Environment(loader=jinja2.FileSystemLoader(self.template_path))

        header_template = env.get_template(header+self.template_extension)
        tailer_template = env.get_template(tailer+self.template_extension)
        body_template = env.get_template(filename+self.template_extension)

        rendered_header = header_template.render(**kwargs)
        rendered_tailer = tailer_template.render(**kwargs)
        rendered_body = body_template.render(**kwargs)

        rendered_template = self.render(
            body_template=rendered_body, 
            header_template=rendered_header, 
            tailer_template=rendered_tailer)
    ...
    # Create a Jinja2 environment and set the directory for templates
